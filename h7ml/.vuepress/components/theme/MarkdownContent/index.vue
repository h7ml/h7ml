<template>
  <MarkdownContent />
  <div
    v-show="author?.frontmatter?.layout !== 'BlogHome' && !author?.frontmatter?.home"
    class="joe_detail__copyright theme-hope-content"
  >
    <div class="content">
      <div class="item">
        ...
        <span>版权属于：</span>
        <span class="text">
          <a :href="href" title="h7ml">{{ author?.frontmatter?.author || 'h7ml' }}</a>
        </span>
      </div>
      <div class="item">
        <svg class="icon" width="20" height="20" viewBox="0 0 1024 1024">
        ...
        <span class="text">
          <a
            class="link"
            :href="'//www.h7ml.cn' + author?.path"
            :title="author?.path"
            target="_blank"
            rel="noopener noreferrer nofollow"
          >
            {{ 'https://www.h7ml.cn' + author.path }}
          </a>
        </span>
      </div>
      <div class="item">
        ...
        <span>作品采用：</span>
        <span class="text" title="署名-非商业性使用-相同方式共享 4.0 国际 (CC BY-NC-SA 4.0)">
          《
          <a
            class="link"
...
<script>
  ...
  export default defineComponent({
    ...
    setup() {
      const author = usePageData();
      const href = `//www.h7ml.cn?${Date.now()}`;
      return {
        author,
        href,
      };
